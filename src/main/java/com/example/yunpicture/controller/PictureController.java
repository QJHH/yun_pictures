...
import com.example.yunpicture.manager.auth.SpaceUserAuthManager;
import com.example.yunpicture.manager.auth.StpKit;
import com.example.yunpicture.manager.auth.annotation.SaSpaceCheckPermission;
import com.example.yunpicture.manager.auth.model.SpaceUserPermissionConstant;
...
//    @AuthCheck(mustRole = UserConstant.ADMIN_ROLE)
    ...
    public BaseResponse<Page<PictureVO>> listPictureVOByPage(@RequestBody PictureQueryRequest pictureQueryRequest,
        ...
        long size = pictureQueryRequest.getPageSize();
        // 限制爬虫
        ThrowUtils.throwIf(size > 20, ErrorCode.PARAMS_ERROR, "最多显示20条数据");
        //空间权限校验
...
//            }
    ...
    /**
     * 分页获取图片列表，封装类、有redis缓存  --->改进成 多级缓存
     *
     * @param pictureQueryRequest
     * @param request
     * @return
     */
    @Deprecated//私有图库更新频率不好把握，可以先不使用缓存分页查询图片 接口
    ...
    public BaseResponse<Page<PictureVO>> listPictureVOByPageWithCache(@RequestBody PictureQueryRequest pictureQueryRequest,
        ...
        // 设置缓存的过期时间，5 - 10 分钟随机过期，防止雪崩（即，同一时间大量缓存失效，都请求数据库，导致数据库崩溃）
