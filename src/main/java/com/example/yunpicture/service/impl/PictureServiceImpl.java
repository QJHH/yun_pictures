...
import com.example.yunpicture.manager.CosManager;
import com.example.yunpicture.manager.FileManager;
import com.example.yunpicture.manager.upload.FilePictureUpload;
import com.example.yunpicture.manager.upload.PictureUploadTemplate;
import com.example.yunpicture.manager.upload.URLPictureUpload;
...
import com.example.yunpicture.utils.ColorTransformUtils;
 ...
 * @createDate 2025-07-30 21:40:22
 */
@Service
@Slf4j
public class PictureServiceImpl extends ServiceImpl<PictureMapper, Picture>
    ...
    @Resource
    private FileManager fileManager;//已弃用
    ...
    public PictureVO uploadPicture(Object inputSource, PictureUploadRequest pictureUploadRequest, User loginUser) {
        ...
        if (spaceId != null) {
            ...
            //改为使用统一的权限校验
            //校验是否有空间的权限,仅有空间管理员才能上传图片，目前空间的管理员就是空间的创建人
...
//            }
            //校验空间是否一致
            //没传 spaceId ，则复用原图片的spaceID，这样也兼容了公共图库
            if (spaceId == null) {
                if (oldPicture.getSpaceId() != null) {
                    spaceId = oldPicture.getSpaceId();
                }
            } else {
                //传了spaceId，必须和原图的空间Id一致
    ...
    public QueryWrapper<Picture> getQueryWrapper(PictureQueryRequest pictureQueryRequest) {
        ...
        //新增编辑时间的查询条件 >= startEditTime  数据库中的列名editTime
    ...
    public void clearPictureFile(Picture oldPicture) {
        ...
        if (oldPicture.getSpaceId() == null) {
            pictureUrl = pictureUrl.substring(pictureUrl.indexOf("public"));
            thumbnailUrl = thumbnailUrl.substring(thumbnailUrl.indexOf("public"));
        } else {
            pictureUrl = pictureUrl.substring(pictureUrl.indexOf("space"));
            thumbnailUrl = thumbnailUrl.substring(thumbnailUrl.indexOf("space"));
        }
        //删除图片webp格式
